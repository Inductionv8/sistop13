...
result = calloc(1, sizeof(scommand_s));
result->list =  g_slist_alloc();
assert(result != NULL);
assert(scomand_is_empty(result);
...
void scommand_destroy (scommand *self){
        assert(self != NULL);
        g_slist_free_full(self->list,bdestroy);
        if (self->in != NULL){
             bdestroy(self->in);    /*el if es por que en el caso que no tenga nada el in y/o out la funcion bdestroy devuelve b_error */
